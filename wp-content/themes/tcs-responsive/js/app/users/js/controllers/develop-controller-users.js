 ...
 */
var DevelopCtrl = function ($scope, UsersService, ChartService) {
  var dataCtrl = this;
  ...
  $scope.$watch('subTrack', function () {
    ...
    if ($scope.stats === undefined) {
      ...
      UsersService.getStatistics(user, $scope.track, $scope.baseCtrl.cache).then(function (stats) {
        dataCtrl.dealWithDevelopStatistics($scope, stats, ChartService, UsersService);
      }, function errorCallback(){
        dataCtrl.dealWithDevelopStatistics($scope, {}, ChartService, UsersService);
      });
    } else {
      dataCtrl.requestRatingAndDistribution($scope, ChartService, UsersService);
    }
 ...
 */
DevelopCtrl.prototype.dealWithDevelopStatistics = function($scope, stats, ChartService, UsersService){
  var dataCtrl = this;
 ...
 */
DevelopCtrl.prototype.requestRatingAndDistribution = function($scope, ChartService, UsersService){
  var dataCtrl = this;
  if ($scope.subTrack) {
    dataCtrl.subTrack = $scope.subTrack;
    ...
    if (dataCtrl[dataCtrl.subTrack] === undefined) {
      UsersService.getRatingAndDistribution(
