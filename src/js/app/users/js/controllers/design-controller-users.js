 ...
 */
var DesignCtrl = function (UsersService, ImageService, $scope, CoderbitsService) {
  var designCtrl = this;

  designCtrl.init();

  CoderbitsService.getDesigns(user).then(function(resp){
    $scope.additionalDesigns = resp;
  });

  //get the recent wins stats asynchrous.
  UsersService.getRecentWins(user, $scope.track, $scope.baseCtrl.cache).then(function (recentWins) {
...
tc.controller('DesignCtrl', ['UsersService', 'ImageService', '$scope', 'CoderbitsService', DesignCtrl]);