...
/**
 * The controller for badges section of users page.
 */
var BadgeCtrl = function ($scope, UsersService) {
  var badgeCtrl = this;
  badgeCtrl.init($scope);
  badgeCtrl.mapBadges();
  $scope.badgeCtrl = this;

  // Get the user's profile. 'user' is defined in ng-page-users.php
  UsersService.getUserId(user).then(function (data){
    badgeCtrl.dealWithBadgeData(data.users[0].userId, $scope, UsersService);
  }, function errorCallback(){
    badgeCtrl.dealWithBadgeData(undefined, $scope, UsersService);
  });
 ...
 */
BadgeCtrl.prototype.dealWithBadgeData = function(userId, $scope, UsersService){
  var badgeCtrl = this;
  /*
      ...
      $($scope.coder.Achievements).each(function(){
        ...
        if(value){
          ...
          if(userId && $.inArray(value.id, excluded_badgesID) <= -1 ){
            UsersService.getAchievementCurrent(userId, value.id).then(function (data){
