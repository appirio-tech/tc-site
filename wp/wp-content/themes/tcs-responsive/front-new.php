...
<?php echo '<script type="text/javascript">
    $(window).load(function(){
        if(!app.isLoggedIn()){
            $(\'.content .article a[href="/?action=callback#"]\').attr("onclick", "$(\'.btnRegister\').click(); return false;");
            
            if(window.location.hash!="")
                $(".pwd, .confirm, .strength").parents(".row").hide();
            $("#register a.btnSubmit").addClass("socialRegister");
        }
        $(\'.content .article a[href="/?action=callback#"]\').attr(\'href\', \'#\');
    });
</script>'; ?>
